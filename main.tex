...
\definecolor{omrComment}{HTML}{595959}   % # ...
...
\begin{frame}{Alternative: ExaModels as a JuMP backend for GPU acceleration}{Idea: a small bucket of fully-typed building blocks}
  \begin{itemize}\itemsep2pt
  \item Don't ask JuMP to be fully type-stable. Give it a small \alert{bucket of building blocks} that are \alert{fully typed and concrete}
  ...
  \begin{itemize}\itemsep2pt
  \item<3-> JuMP authors against these blocks; ExaModels \alert{dispatches statically} on the common patterns and falls back to per-element type inference only for the \texttt{Any} bucket
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  ...
  \visible<4->{
    \begin{block}{Discussion}
      \scriptsize We don't need to move all of JuMP onto the GPU. The win is \alert{GPU-native callbacks} eliminating the per-iteration round-trip. A slightly more type-stable JuMP tree would make the bridge cheap.
    \end{block}
  }
  \visible<5->{
    \begin{exampleblock}{Continuing this thread at SIAM Optimization 2026}
      \scriptsize ``The State of ExaModels'' --- MS369, \alert{Fri June 5, 11:10 BST}.
    \end{exampleblock}
  }
\end{frame}
